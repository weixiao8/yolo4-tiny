...
class Retinaface(object):
    ...
    def __init__(self, encoding=0, **kwargs):
        ...
        try:
            self.known_face_encodings = np.load("model_data_face/{backbone}_face_encoding.npy".format(backbone=self.facenet_backbone))
            self.known_face_names     = np.load("model_data_face/{backbone}_names.npy".format(backbone=self.facenet_backbone))
        except:
            if not encoding:
    ...
    def encode_face_dataset(self, image_paths, names):
        ...
        np.save("model_data_face/{backbone}_face_encoding.npy".format(backbone=self.facenet_backbone),face_encodings)
        np.save("model_data_face/{backbone}_names.npy".format(backbone=self.facenet_backbone),names)
